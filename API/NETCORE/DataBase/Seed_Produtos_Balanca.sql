/**
 * Arquivo: API/NETCORE/DataBase/Seed_Produtos_Balanca.sql
 * Objetivo: Carga inicial/importação de todos os 133 produtos da balança etiquetadora (Triunfo Quantum 30 T)
 *           com códigos PLU, nomes, unidade (KG / BDJ / UN) e estoque inicial.
 * Idempotente: pode ser executado várias vezes sem duplicar nem gerar erro.
 */

SET NOCOUNT ON;

DECLARE @CompanyId NVARCHAR(40) = N'empresa-principal';

-- Tabela temporária com os produtos extraídos das tabelas da balança
...
-- Executa o MERGE para inserir ou atualizar mantendo integridade
MERGE INTO Produtos AS target
USING #ProdutosBalancaTemp AS source
ON target.CompanyId = @CompanyId AND target.ProductCode = source.ProductCode
WHEN MATCHED THEN
    UPDATE SET 
...
WHEN NOT MATCHED THEN
    ...
    (
        ...
        9999.0000, -- Estoque inicial para permitir vendas diretas no PDV sem travar por estoque
        0.0000,
...
SELECT 
    COUNT(1) AS TotalCadastrados 
FROM Produtos 
WHERE CompanyId = @CompanyId AND ProductSupplier = N'Balança Etiquetadora';
